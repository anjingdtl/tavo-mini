...
状态：`C9 GO`。C8 已在修正版 APK 上完成真实 Compact Clean 与 force-stop/relaunch 保护；C9 只封成本、时延、稳定性统计口径，不重复 C3-C8 的已 GO 工作，也未把统计模块接入生产 Governor 决策。
...
#### ACT / C9 GO（2026-08-30）

- CHECK-B 已通过：`npm.cmd run apk:debug` PASS；APK 为 `dist/apk/debug/ShineWriter-V2.21.1-debug.apk`，SHA-256=`5CBE79A7E29B0DF393F092DA7AB52084282EC6F6A64FB8688C28C6753115D42A`，大小 `59,879,950` bytes，`versionCode=2210100`、`minSdk=24`、`targetSdk=36`。`adb devices` 核验 `emulator-5554 device`；只执行 `adb -s emulator-5554 install -r`，结果 `Success`。`firstInstallTime=2026-08-23 04:59:45` 前后不变；没有 uninstall、`pm clear`、清库或重置真实 LLM 配置。摘要：`test-logs/phase3-c9-cost-latency-20260830-000001/c9-apk-install-summary.txt`。
- 新 APK 上完成真实 Android run `ct_6b677834d7c24d22910f487fe0264e39`：使用 App 已配置的真实 `open.bigmodel.cn-v4 / GLM-5.3-Flash`，Thinking=`enabled`，Frozen/provider effort=`high/high`，capability=`supported`。语义 UI 完成新建章节、AI 确认、生成详情与 `采纳`；结果显示“与初稿一致 / 2,006 字 / 10 段”，采纳后编辑器显示“已保存”。证据：`c9-ai-dialog-2.xml/.png`、`c9-ai-running.xml/.png`、`c9-ai-result.xml/.png`、`c9-result-details.xml/.png`、`c9-adopted.xml/.png`。
- 稳定只读 SQLite `c9-final.sqlite` / `c9-final-projection.json`：`dbIntegrity=ok`，`api_key=0 / authorization=0 / bearer=0`，`rawPromptOrBodyStoredInProjection=false`。Draft Receipt 为 `physical=1`、`wireMaxTokens=25221`、`finishReason=stop`、`outcome=succeeded`，usage `input/output/reasoning/visible=42081/17851/15534/2317`，timings `queue/provider/parse/persist/total=0/299472/2/25/299499ms`；QA Receipt 为 `physical=1`、`wireMaxTokens=23324`、`finishReason=stop`、`outcome=succeeded`，usage `24349/1455/1324/131`，timings `0/25664/1/0/25666ms`。Revision 正式跳过，FinalValidate `0` request；run total `physical=2`、`85736` tokens、postWriting auxiliary=`0`、Governor physical=`0`、retry=`0`、fallback=`0`。Receipt 摘要：`test-logs/phase3-c9-cost-latency-20260830-000001/c9-receipt-summary.txt`。
- 产品进程 PID `27704` 的 `c9-logcat.txt` 过滤扫描中，`FATAL EXCEPTION / ANR / process death / OOM / CRASH / SIGSEGV` 均为 `0`；原始 logcat 中已保留已知非致命 React reinitialization soft-exception 行，没有把它们伪装成零日志。证据与 Android gate 摘要：`test-logs/phase3-c9-cost-latency-20260830-000001/c9-android-gate-summary.txt`。
- C9 aggregate 按“每个 unique run 只取一个 final safe projection、排除 poll snapshot”执行，覆盖 C3-C8 与 C9 的 `23` 个真实 Fast/Standard case；Quality 的真实证据（包括 C7 parse/invalid-format 失败）单独保留，不错标为 Standard。共 `37` 个 Receipt、`38` 个 paid-request denominator：C8 已知 `outcome_unknown` reservation 以 ledger-only `1` 个未具名物理调用计入，不伪造 Receipt。聚合结果 `writerPhysicalCalls=38`、`totalPaidCalls=38`、postWriting auxiliary=`0`、Governor=`0`、protocol fallback=`0`；queue p50/p95=`0/1ms`，provider p50/p95=`187740/337781ms`，total p50/p95=`187764/337786.8ms`；input p50/p95=`38125/42615`、output=`11435/17017.4`、reasoning=`9587/15633.8`、visible=`1592/2348.2`；dynamic output/wire utilization p50/p95=`0.2404/1`，缺失样本=`1`（未被当成零）。错误率按 paid denominator：timeout=`0/38`、outcome_unknown=`1/38=0.0263`、length=`5/38=0.1316`、Fast/Standard exact-set invalid-format=`0/38`。这些旧 `length`/`outcome_unknown`/Quality parse 失败均原样保留，未被统计模块改写或自动重试。完整标量结果：`test-logs/phase3-c9-cost-latency-20260830-000001/c9-aggregate.json`。
