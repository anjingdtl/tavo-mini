...
3. 生成失败、取消、截断、结构无效或回读失败时，绝不保存文件、绝不写入资料库；仅质量目标未达成但结构可回读的产物必须保留到预览，由用户决定保存、导入或重试。
...
任何一个可见内容校验表明产物低于当前 `requiredMinOutput`，都在 `qualityReport` 中标记为“生成规模不足”，但不再丢弃已经通过结构、必填字段与回读校验的产物。Token 总量和字段长度用于生成目标、预算规划与补强提示，不作为保存 / 导入的一票否决条件。

预览提示必须包含可操作信息，例如：

```text
未完全达到“丰满”目标，已保留本次结果。
实际约 1,920 / 目标 2,800 Token。仍可预览、保存或导入；如需更丰富内容，可重新生成。
```

不自动发起隐藏的第二次模型调用。预览区显示非阻断质量提示，并继续提供：
...
维度覆盖、Token 总量、字段长度、对话轮数和标签数量只用于解释与补强提示，不应把结构完整、能够回读的角色卡整份丢弃。硬性拒绝条件只包括：响应截断、JSON / 必填结构无效、核心文本全空、世界书条目数或关键词无效，以及回读不兼容。
...
按当前丰满度档位检查每条 `content` 的中文有效字符数量；不足时保留完整条目并在质量报告中提示，不能静默删除短条目后减少条目数。条目正文为空、条目数不符或回读失败仍硬阻断。
...
3. TXT 来源摘要及片段勾选列表；
4. 预算面板中的“生成目标”“本次输出预留”“来源预算”“预计输入”；
...
“保存到手机”“导入资料库”在结构和回读校验通过的预览状态中可见；质量目标未完全达到时同时显示非阻断提示。
...
`constructionFileService.ts` 的文件序列化和直接导入资料库逻辑保持为既有入口；它只接收已经通过生成器结构与回读校验、并携带质量报告的 `ConstructionArtifact`。
...
通过 JSON 结构、必填字段和既有导入回读校验后函数即可 resolve；`qualityReport.passed` 只表示是否完全达到所选质量目标。未达目标时必须把 report 与 artifact 一起返回，让预览显示差距并保留用户选择权，而不是让页面重复计算质量或丢弃有效结果。
...
10. 质量目标未达成仍返回可保存 artifact，并携带 `passed=false` 与可解释差距；结构或回读失败不返回 artifact。
...
5. 可见产物低于 Token 目标时返回 `passed=false` 的质量报告，但不拒绝结构可回读的产物；provider usage 不参与放行；
...
5. 超预算 TXT 禁止生成；
6. 世界书预览明确显示“全部常驻”；
7. 质量目标未达成时仍有保存/导入按钮，并显示实际值、目标值与补强项；
