...
- 长候选正文新增可拖动、可点击轨道的纵向滑动条，与正文手势滚动双向同步；短正文不显示滑动条。
- 保留长按选择的 UTF-16 范围；拖动滚动条、切换修订要求输入焦点及父组件重渲染后，生成预览仍只修订选中内容。
- 增加长短正文、滚动映射、正文手势同步和选区预览回归测试；升级版本至 `V3.0.9` / `versionCode=3000900`。

## [3.0.8] - 2026-09-13

### Release — 模拟器 UI 全量验收与发布封板

- 完成 Android 模拟器 UI 全量验收，覆盖启动、导航、核心编辑与资料库交互路径。
- 确认结果操作区防溢出与修订选择保持修复已纳入发布版本，并完成对应回归验证。
- 升级版本元数据至 `V3.0.8` / `versionCode=3000800`，生成正式签名 APK 与更新元数据。

## [3.0.7] - 2026-09-11

### Fixed — 资料库全 Tab 滚动架构统一

- 修复资料库作家风格页顶部说明、内置作家风格目录与展开预览被固定在屏幕上，导致下方用户作家风格不可达、页面无法纵向滚动的问题；改为单一 FlatList 滚动容器，内置目录与展开预览进入滚动链，用户作家风格继续使用 FlatList 虚拟化。
- 统一资料库各 Tab 滚动所有权：角色、世界书、笔记、作家风格均为单 FlatList 滚动 owner，动态增长区域一律进入 ListHeaderComponent；为角色、世界书与作家风格列表补充稳定 testID，固化滚动所有权不变量。
- 新增资料库全 Tab 滚动可达性回归门禁（作家风格 WS-1~7 与四 Tab R-1~5），覆盖 100 条作家风格虚拟化、最后一条操作、编辑器打开与目录复制等场景，防止滚动结构回归。

## [3.0.6] - 2026-09-11

### Fixed — 笔记资料库滚动与统计回归

- 修复资料库笔记页顶部仿写/资料库配置区挤压列表导致无法继续滚动的问题，改为单一 FlatList 滚动容器并保留大量笔记虚拟化。
- 恢复禁用模式下的已导入笔记总数、项目可用数和当前参与数的清晰区分。
- 补充 200 条笔记、合集层级、Switch 和滚动结构回归测试。

## [3.0.5] - 2026-09-11

### Fixed — Construction 独立构建补充需求隔离

- 修复从 TXT 模式切换到独立角色或世界书构建后，隐藏的补充需求仍被带入生成请求的问题。
- 保留仍显示补充需求的构建模式及旧调用方兼容性，并补充 UI payload 与生成 Prompt 回归测试。

## [3.0.4] - 2026-09-11

### Fixed — 大型数据库升级与角色卡导入安全

- Schema 61 → 62 识别为 `schema_only` 轻量迁移，不再因新增 `llm_config.vision_support` 扫描章节/笔记正文、生成整库恢复备份或执行全库内容指纹；高风险历史迁移仍保留完整保护链。
- 增加迁移风险分级、启动诊断、批量指纹读取和有界 dirty-marker 恢复，覆盖中断升级、结构漂移和大数据库回归场景。
- AI 角色视觉参考图恢复为单张不超过 5 MB；历史 PNG 角色卡导入不复用该上限，兼容包含大体积嵌入数据的旧 PNG。
- PNG 角色卡单个及批量导入在数据库写入失败时删除新建的永久图片，避免产生孤儿资源；角色换图继续遵循先保存新图、数据库更新成功后再删除旧图。

## [3.0.3] - 2026-09-11

### Added — 角色视觉资产与安全多模态构建

- 支持为角色资料导入、替换、预览和删除 JPEG、PNG、WebP 视觉参考图，并将图片纳入角色资料生命周期管理。
- 独立角色构建可在视觉模型能力确认后携带参考图请求多模态模型；不支持或未知模型默认拒绝发送图片，避免静默丢失视觉上下文。
- 参考图导入上限提升至 20 MB，增加格式、尺寸、持久化、回滚和资源清理校验。
- 完成 Schema 62 迁移、LLM 能力配置和端到端回归测试覆盖。

## [3.0.2] - 2026-09-09

### Fixed — 资料选择稳定性与系统更新下载

- 修复大规模笔记选择在刷新、连续点击、切换项目和重启后出现回滚或串项目的问题，并为 49/50/51/100/200 条笔记场景增加稳定选择与虚拟列表回归覆盖。
- 移除笔记批量导入沿用的 50 文件上限，改为顺序导入，保留角色卡和世界书的产品限制。
- GitHub APK 更新改由 Android DownloadManager 托管，持久化任务状态并支持通知、暂停/恢复、进程重启恢复和 `content://` 结果安全落盘。
- 保留 SHA-256、包名、版本号、正式签名证书和未知来源权限校验，校验失败拒绝安装。

## [3.0.1] - 2026-09-08

### Fixed — 已定稿章节手动编辑后的状态闭环

- 已定稿章节正文发生真实修改时立即回到草稿并要求重新定稿；相同正文保存仍保持定稿状态。
- 补强自动保存、修订历史和 Story Memory 更新的持久化闭环，并补充相关回归测试。
...
- V5 作家风格快照在序列化/解析往返后保持版本、阶段投影和执行快照字段。
- 作家风格活动决议统一为空绑定、悬空绑定和正常绑定三态。
- 删除无 WHERE 的遗留全局 Context Auto 写库入口；`none/禁用` 笔记模式不再注入正文。
- 多章批次自动消耗上限按章数扩展，旧批次冻结上限保持不变。
...
- **批量写 N 章：批次模式选择真实进入单章执行**（RB-1）：创建页的「仅草稿 / 快速 / 完整」此前只进入规划器，单章 Pipeline 实际按全局设置执行（选「仅草稿」仍会跑完整流水线）；现在通过 `pipelineModeOverride` 在首次冻结执行快照时生效（仅草稿→noReview、快速→twoStage），恢复时仍使用首次冻结值；进度条按批次模式计算阶段。
- **批量写 N 章：采用崩溃窗口导致正文丢失**（RB-2）：`adoptPipelineTaskResult` 中「旧正文修订已写入、章节正文尚未写入」时进程被杀，恢复后幂等检查（最新修订 source_ref 命中）短路跳过正文写入，批次仍推进计数 → 章节正文为空却被记为成功；短路条件增加「正文已实际落库」校验，并避免重复创建旧正文修订。
- **批量写 N 章：项目尾部漂移保护真实生效**（RB-3）：`startPosition` / `expectedTailChapterId` 此前从未在生产路径写入（仅测试 fixture 手工设置），漂移检测是死代码；现在保存计划（确认）时冻结尾部锚点，运行中用户插入/删除章节会暂停批次等待确认；测试 fixture 同步改为走生产语义。
- **批量写 N 章：批次用量按真实请求统计**（RB-4）：`used_llm_calls` 此前只累计成功阶段（每个阶段至多 1 次），失败/重试/格式修复/reasoning 重试请求全部漏算；现在以 `pipeline_stage_attempts` 为审计来源（每次 HTTP 请求一行，成功行带 token）。
- **批量写 N 章：冷启动批次归一化**（RB-5）：应用被杀后批次保持 `running` 且无人执行（UI 显示运行中却永不前进）；冷启动把过期租约的活跃批次落为 `paused_user`（“应用中断，请确认后继续”），不再出现“无执行器的运行中”。
- **批量写 N 章：暂停立即中断当前请求**（RB-6）：此前点「暂停」只改状态，当前章 LLM 请求继续跑完（费用继续产生）；现在暂停会 abort 在飞请求，checkpoint 落为 `interrupted`，恢复时复用已成功阶段。
- **批量写 N 章：租约逐步续租 + 长请求心跳**（RB-7）：60s 租约在整章生成（数分钟）期间会过期，第二个执行器可进入；现在状态机每步续租、单章请求期间心跳续租，失去租约即停止推进。
- **批量写 N 章：点击开始立即进入运行页**（RB-8）：`start` 此前阻塞整个 reconcile，界面停在预览页直到第一章完成才切运行页（期间开始按钮可重复点击）；现在非阻塞驱动 + 立即切页 + `reconciling` 防重入，运行页 2s 轮询刷新。
- **批量写 N 章：重启后恢复为「确认后继续」而非「线程被占用」**（RB-9）：点击开始后批次状态在第一章采用前仍为 `ready`（仅采用时切 `running`），快速重启时死进程的租约（60s 内）与 `ready` 批次都不会被冷启动归一化 → 再次点击「开始批量写作」报「批次已被其他执行器占用」；现在冷启动无条件清理死进程租约，并把 `running`/`waiting_retry` 与「已有执行痕迹（已建章节/已绑任务）」的 `ready` 批次统一落为可恢复暂停，页面显示「应用中断，请确认后继续」，确认后沿幂等路径续写（已完成的章节/阶段不会重跑）。
- **批量写 N 章：运行中的批次不再掉回预览/创建页**（RB-11）：批次状态在第一章采用前一直是 `ready`，此前协调器交还控制权（等待重试）或页面切走再切回时，视图会掉回预览页甚至创建页（误导用户以为批次没开始）；现在只要协调器在驱动或有执行痕迹就保持运行页，仅全新批次显示预览页。
- **批量写 N 章：安全重试到期后自动恢复**（RB-12）：`wait_until` 等待未到期时协调器提前交还控制权，此后批次停在运行状态但无人执行（safe_retry 自动重试实际不生效）；运行页轮询现在充当看门狗，`waiting_retry` 到期且无协调器时自动重新驱动。
