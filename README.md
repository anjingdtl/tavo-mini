...
[![Version](https://img.shields.io/badge/Version-V2.5.5-blue.svg)](CHANGELOG.md)
...
当前版本：**V2.5.5** · 数据库 Schema：**15** · 最低 Android API：**24**

`V2.5.5` 为长篇定稿加入 JSON Object 输出、截断识别和三级动态扩容，避免人物关系图与鱼骨故事线增长后模型返回不完整 JSON。定稿成功仍保证非空章节事件摘要；全量人物实体表、关系图以及并行冲突/线索/伏笔会注入后续章节。Schema 15 保持不变。
...
当前正式产物：`dist/apk/release/ShineWriter-V2.5.5-release.apk`（37,264,135 bytes），SHA-256：`5E69E3A1E64F489EE8A33DF776505238BCD7FB79BFE3CF9A8F5DB12A74E66319`。
...
V2.5.1 的结构化记忆基线、覆盖率和未完成真机矩阵记录在 [`docs/V2.5.1-STORY-MEMORY-TEST-REPORT.md`](docs/V2.5.1-STORY-MEMORY-TEST-REPORT.md)；V2.5.2–V2.5.5 的 DeepSeek 真机问题复现与模拟器 release 回归记录在 [`docs/RELEASE_CHECKLIST.md`](docs/RELEASE_CHECKLIST.md)。覆盖率门禁为全局 branches `55%`、functions `65%`、lines `65%`、statements `65%`，Schema、迁移、数据库和备份服务有更高的定向阈值。
...
The current version is **V2.5.5** with database Schema **15**. Structured chapter finalization uses JSON mode, detects length-truncated responses, and retries with progressively larger output budgets while preserving atomic memory updates and non-empty episodic summaries. The app stores SQLite data and local models on-device. API keys remain in Android Keystore and are excluded from backups.
