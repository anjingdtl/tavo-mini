...
[![Version](https://img.shields.io/badge/Version-V2.10.6-blue.svg)](CHANGELOG.md)
...
当前版本：**V2.10.6** · 数据库 Schema：**25** · 最低 Android API：**24**

`V2.10.6` 明确区分大纲创作与原著续写：续写工作台和资料入口直接展示 Canon 自动调度边界；角色卡、世界书、笔记、预设仅在明确标记为“外部补充”时注入，不会重复引用原著镜像；章节上下文预览展示实际续写组装结果。详见 [CHANGELOG](CHANGELOG.md)。
...
- SQLite：数据库文件名为 `shine_writer.db`，位于 Android 应用私有数据目录，当前 Schema 为 24。
...
下一正式产物：`dist/apk/release/ShineWriter-V2.10.6-release.apk`，`versionName=V2.10.6`，`versionCode=2100600`。

V2.10.6 将按 [Release APK 构建指南](docs/RELEASE_APK_BUILD.md) 执行 `npm run apk:release` + `apksigner verify` + `zipalign -c` + `aapt dump badging` 全套验收。
...
| versionName / versionCode | V2.10.6 / 2100600（构建后核验） |
| 文件大小 | 38,245,047 bytes（36.47 MB） |
| APK SHA-256 | `FCF9D37214CE320E4220BEBBC926372B02ACBC89A2731223431B72302F962AEB` |
| Android 模拟器验收 | Android 17 x86_64：大纲创作与原著续写模式入口、续写工作台、资料注入边界与章节上下文预览回归通过 |
...
The current version is **V2.10.6** with database Schema **25**. V2.10.6 separates outline creation from Canon-driven continuation and makes ordinary resources opt-in external supplements, preventing duplicate source injection. The app stores SQLite data and local models on-device. API keys remain in Android Keystore and are excluded from backups.
