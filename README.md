...
The current version is **V2.5.16** with database Schema **16**. Story memory uses a checkpoint architecture (smart interval, typically every 3 chapters) with Checkpoint + Pending Bridge + Seam context, soft-skip for missing entity refs, and atomic local finalization before long-term memory. V2.5.16 engineering-reliability closure: illegal target chapter positions hard-block `prepareStoryMemoryForGeneration()` before coverage / Episodic / generation; `invalid_position` now carries `invalidPositionSource` (`target` | `checkpoint`) with accurate trace copy; the Release APK main script reuses `Test-ApkSignerAcceptance` as the single acceptance entry; README documents the target APK filename without claiming a signature-verified artifact exists. The app stores SQLite data and local models on-device. API keys remain in Android Keystore and are excluded from backups.
