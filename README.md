...
如果你已经有完整原著，也可以创建“原著续写”项目：导入原著、设置续写边界、完成 Canon 分析后，再从边界之后继续写新的章节。本地数据库当前为 **Schema 44**（含流水线阶段 checkpoint、持久化 LLM Attempt、弹性上下文预算池与批量写章批次表；Schema 43 为旧项目 Smart 故事记忆策略间隔一次性统一迁移，Schema 44 为任务与批次增加 Workflow/Budget 版本冻结列——新任务默认启用大纲写作 V2 与弹性预算 V2，升级前的旧任务与旧批次自动按 V1 恢复）。
