...
[![Version](https://img.shields.io/badge/Version-V2.11.37-blue.svg)](CHANGELOG.md)

</div>

当前版本：**V2.11.37** · 支持 Android 7.0 及以上设备
...
如果你已经有完整原著，也可以创建“原著续写”项目：导入原著、设置续写边界、完成 Canon 分析后，再从边界之后继续写新的章节。本地数据库当前为 **Schema 43**（含流水线阶段 checkpoint、持久化 LLM Attempt、弹性上下文预算池与批量写章批次表；Schema 43 为旧项目 Smart 故事记忆策略间隔一次性统一迁移）。
...
正式 APK 文件：`dist/apk/release/ShineWriter-V2.11.37-release.apk`

构建标识：`versionName=V2.11.37`，`versionCode=2113700`。
...
The current version is **V2.11.37**. ShineWriter is an Android-first writing workspace. It supports outline writing, Canon-driven continuation, optional online AI, backups, and text-to-speech.
