...
describe('schema 26 versioned continuation style profiles', () => {
  ...
  it('rebuilds the legacy table and marks rows outdated on upgrade', () => {
    ...
    // analysis_runs is rebuilt to widen the stage CHECK.
    expect(joined).toContain('RENAME TO continuation_analysis_runs_v25');
    expect(joined).toContain("'style_analysis'");
    expect(joined).toContain("'style_validation'");
    expect(joined).toContain('DROP TABLE continuation_analysis_runs_v25');
