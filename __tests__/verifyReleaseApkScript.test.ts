...
function parseApkSignerOutput(output: string): {
  ...
  const verifiedAnySchemeLine = lines.some(l =>
    /^Verified\s+using\s+v\d+\s+scheme:/i.test(l),
  );
  const verified = verifiedV2 || verifiedAnySchemeLine;
  ...
  return { verified, verifiedV2, numberSigners, certSha256 };
}
...
describe('PowerShell parsing helpers — TS mirror (pure function coverage)', () => {
  ...
  it('rejects when v2 scheme is false even if signer count is 1', () => {
    const sample = [
