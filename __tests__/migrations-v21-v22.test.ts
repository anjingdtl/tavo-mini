...
describe('schema 22 Canon material work-item migration', () => {
  it('creates the five-family resumable work-item table', () => {
    ...
    const sql = buildV21toV22Statements().map(item => item.sql).join('\n');
    expect(sql).toContain('CREATE TABLE IF NOT EXISTS continuation_analysis_work_items');
  ...
  it('keeps fresh schema and backup manifest aligned', async () => {
    ...
    expect(
      SCHEMA_MANIFEST.find(table => table.name === 'continuation_analysis_work_items'),
