...
export const RevisionHistoryScreen: React.FC<Props> = ({ targetType, targetId, projectId, onClose }) => {
  ...
  const renderItem = ({ item }: { item: ContentRevision }) => {
    ...
    return (
      <Card>
        ...
        <Text style={[styles.wordCount, { color: theme.colors.textMuted }]}>{wordCount(item.content)} 字</Text>
        ...
        <View style={styles.actionRow}>
          ...
          />
          {isRestoring && <Text style={[styles.restoring, { color: theme.colors.textMuted }]}>恢复中…</Text>}
        </View>
      </Card>
    );
  };
  ...
  return (
    <Screen>
      <Header
        ...
        action={<Button label="关闭" variant="ghost" compact onPress={onClose} />}
      />
      {loading ? (
        <LoadingState />
      ) : revisions.length === 0 ? (
        <EmptyState title="暂无历史版本" description="编辑内容时会自动保存历史版本" />
      ) : (
        <FlatList
          data={revisions}
