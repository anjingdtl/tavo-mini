...
function hardFactsBlock(view: ContinuationV5StageViews['draft_writer']): string {
...
export function compileContinuationV5AuditorMessages(input: {
  ...
  const system = [
    ...
    '你与 Revision Writer 并行，不读取 V2。',
    '你独立审查 V1 + A1 + 冻结资料，输出最终修订合同 C2。',
  ...
  const user = [
    ...
    `【完整 V1】\n${input.draftContent}`,
    `【A1】\n${JSON.stringify(input.architecture)}`,
    '【输出】严格包含 canonAudit/styleAudit/architectureAudit/finalObligations 分区，并回显全部绑定字段。',
  ].join('\n\n');
  const messages: ChatMessage[] = [
    { role: 'system', content: system },
    { role: 'user', content: user },
