...
import { callWritingStageLLM } from '../writing/stages/stageLlmCall';
import {
...
export async function reconcilePipelineTask(
  ...
  try {
    ...
    for (let step = 0; step < 32; step++) {
      ...
      const handled = await runOutlineWritingCapability({
        taskId,
        chapter,
        action,
        stages,
 ...
 */
export async function runOutlineWritingCapability(params: {
  taskId: string;
  chapter: Chapter;
  action: PipelineAction;
  stages: ReturnType<typeof resolveStageCheckpoints>;
...
async function callReadyLLM(
  ...
  return callWritingStageLLM(ready.messages, maxTokens, config, abortSignal);
}
